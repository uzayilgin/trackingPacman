    ...
    for f in factors:
        for v in f.unconditionedVariables():
            if v not in unconditionedVariables:
                unconditionedVariables.append(v)
        for v in f.conditionedVariables():
            if v not in conditionedVariables:
                conditionedVariables.append(v)

    updatedConditionedVariables = []
    for v in conditionedVariables:
        if v not in unconditionedVariables:
            updatedConditionedVariables.append(v)
    conditionedVariables = updatedConditionedVariables

    variableDomainsDict = factors[0].variableDomainsDict()
    join = Factor(unconditionedVariables, conditionedVariables, variableDomainsDict)

    for i in join.getAllPossibleAssignmentDicts():
        p = 1
        for factor in factors:
            p *= factor.getProbability(i)
        join.setProbability(i, p)
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        for a in assignments:
            ...
            for val in variableDomainsDict[eliminationVariable]:
                previousAssigments = []
                for k, v in a.items():
                    previousAssigments.append((k, v)) 
                previousAssigments.append((eliminationVariable, val))  

                previousAssignDict = dict(previousAssigments)
                p += factor.getProbability(previousAssignDict)
