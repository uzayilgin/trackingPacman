...
def constructBayesNet(gameState: hunters.GameState):
    ...
    noisy_distances = []
    for i in range(max_distance + MAX_NOISE + 1):
        noisy_distances.append(i)

    variableDomainsDict = {
        PAC: positions,
        GHOST0: positions,
        GHOST1: positions,
        OBS0: noisy_distances,
        OBS1: noisy_distances,
    }
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            result = joinFactorsByVariable(factors, var)
            unjoined_factors = result[0]
            joined_factors = result[1]
            factors = unjoined_factors
            
            if len(joined_factors.unconditionedVariables()) > 1:
                eliminated_factor = eliminate(joined_factors, var)
                factors.append(eliminated_factor)
        final_join = joinFactors(factors)
        normalized_final_join = normalize(final_join)
        return normalized_final_join
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for key, value in self.items():
            current_total_weight += value
            if current_total_weight >= threshold:
                return key
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for p in allPositions:
            ...
            self.beliefs[p] = self.beliefs[p] * observationProbability
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for previousPosition in self.allPositions:
            transitionModel = self.getPositionDistribution(gameState, previousPosition)

            for newPosition, transitionProbability in transitionModel.items():
                updatedBeliefs[newPosition] += self.beliefs[previousPosition] * transitionProbability
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        if weightDistribution.total() == 0:
            ...
            newParticles = []
            for i in range(self.numParticles):
                sampledParticle = weightDistribution.sample()
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        possiblePositions = []
        for ghostPositions in itertools.product(self.legalPositions, repeat=self.numGhosts):
            possiblePositions.append(ghostPositions)

        random.shuffle(possiblePositions)

        totalPositions = len(possiblePositions)

        for i in range(self.numParticles):
            positionIndex = i % totalPositions
            particle = possiblePositions[positionIndex]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.particles:
            ...
            if p in weightDistribution:
                weightDistribution[p] = weightDistribution[p] + weight
            else:
                weightDistribution[p] = weight

        totalWeight = weightDistribution.total()
        if totalWeight == 0:
            self.initializeUniformly(gameState)
        else:
            newParticles = []
